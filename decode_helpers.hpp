...
inline void parse_16b_inst(const IDReg& D, EXReg& e)
{
    uint8_t opcode = (uint8_t)getbits(D.inst, 0, 2);
    uint8_t funct3 = (uint8_t)getbits(D.inst, 13, 3);
    uint8_t funct2;
    switch (opcode) {
    case 0x0:
        switch (funct3) {
        ...
        case 0x2:  // lw => lw rd', offset[6:2](rs1')
            ...
            e.funct3 = 0x2;
            e.rd = 8 | getbits(D.inst, 2, 3);
            e.rs1 = 8 | getbits(D.inst, 7, 3);
            e.imm = getbits(D.inst, 5, 1, 6) | getbits(D.inst, 6, 1, 2) |
                    getbits(D.inst, 10, 3, 3);
            break;
        case 0x3:  // ld => ld rd', offset[7:3](rs1'), zero-extended
            ...
            e.rd = 8 | getbits(D.inst, 2, 3);
            e.rs1 = 8 | getbits(D.inst, 7, 3);
            e.imm = getbits(D.inst, 5, 2, 6) | getbits(D.inst, 10, 3, 3);
            break;
        case 0x6: // sw => sw rs2', offset[6:2](rs1')
            e.opcode = OP_STORE;
            e.funct3 = 0x2;
            e.rs2 = 8 | getbits(D.inst, 2, 3);
            e.rs1 = 8 | getbits(D.inst, 7, 3);
            e.imm = getbits(D.inst, 5, 1, 6) | getbits(D.inst, 6, 1, 2) |
                    getbits(D.inst, 10, 3, 3);
            break;
        case 0x7: // sd => sd rs2', offset[7:3](rs1')
            ...
            break;
        default:
            throw_error("unknown compressed inst: %x", D.inst);
        }
        break;
    ...
    case 0x1:
        ...
        case 0x0:  // addi => addi rd, rd, nzimm[5:0]
            ...
            e.opcode = OP_RI;
            e.imm = getbits(D.inst, 2, 5, 0) | getbits(D.inst, 12, 1, 5);
            e.imm = sign_extend(e.imm, 6);
            e.rd = e.rs1 = getbits(D.inst, 7, 5);
            break;
        ...
        case 0x3:  // addi16sp, lui
            ...
            if (e.rd == 2) {
                ...
                e.opcode = OP_LUI;
                e.imm = getbits(D.inst, 2, 5, 12) | getbits(D.inst, 12, 1, 17);
                e.imm = sign_extend(e.imm, 18);
            }
            break;
        case 0x4:
            funct2 = getbits(D.inst, 10, 2);
            switch (funct2) {
            case 0x0:  // srli => srli rd', rd', shamt[5:0]
                e.opcode = OP_RI;
                e.funct3 = 0x5;
                e.rd = e.rs1 = 8 | getbits(D.inst, 7, 3);
                e.imm = getbits(D.inst, 2, 5, 0) | getbits(D.inst, 12, 1, 5);
                e.alu_op = ALU_SRL;
                break;
            ...
            case 0x3:
                ...
                e.rs2 = 8 | getbits(D.inst, 2, 3);
                try {
                    uint8_t funct = getbits(D.inst, 5, 2);
                    uint8_t funct1 = getbits(D.inst, 12, 1);
        ...
        case 0x5:  // j => jal x0, offset[11:1]
            e.opcode = OP_JAL;
            e.imm = getbits(D.inst, 2, 1, 5) | getbits(D.inst, 3, 3, 1) |
                    getbits(D.inst, 6, 1, 7) | getbits(D.inst, 7, 1, 6) |
                    getbits(D.inst, 8, 1, 10) | getbits(D.inst, 9, 2, 8) |
                    getbits(D.inst, 11, 1, 4) | getbits(D.inst, 12, 1, 11);
        ...
        case 0x7:  // bnez => bne rs1', x0, offset[8:1]
            ...
            e.rs1 = 8 | getbits(D.inst, 7, 3);
            e.imm = getbits(D.inst, 2, 1, 5) | getbits(D.inst, 3, 2, 1) |
                    getbits(D.inst, 5, 2, 6) | getbits(D.inst, 10, 2, 3) |
                    getbits(D.inst, 12, 1, 8);
    ...
    case 0x2:
        ...
        case 0x0:  // slli => slli rd, rd, shamt[5:0]
            ...
            e.alu_op = ALU_SLL;
            e.rd = e.rs1 = getbits(D.inst, 7, 5);
            e.imm = getbits(D.inst, 12, 1, 5) | getbits(D.inst, 2, 5);
            break;
